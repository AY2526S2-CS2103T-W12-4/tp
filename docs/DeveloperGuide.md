...
:bulb: **Tip:** The `.puml` source files used to generate the diagrams in this guide are located in the `docs/diagrams` folder.
...
* manages **View Mode**, a UI state where full contact details are displayed in a dedicated detail panel. Refer to the
...
calls `handleViewPerson()` to display the selected contact's full details in a dedicated detail panel while hiding the normal list panel.
...
6. **Support undo/redo**: Currently, FAM does not support undo/redo. In a future version, we plan to add undo/redo using a versioned state history so that users can revert or reapply recent changes safely.